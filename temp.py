...
from functions import parse_B1_txt, get_S1, moving_average, del_V_S, truncate_win2, interp_pr, truncate_ch, \
    get_scatter_coef, magnific_ch, get_attr, get_win_std
...
def get_p_pos(lead1, lead2, lead3, intervals, r_pos, chars, inds_min):
    ...
    return presence_PR1, presence_PR2, presence_PR3, marr_amp_p1, marr_amp_p2, marr_amp_p3, mean_PR, mean_PR1, mean_PR2, mean_PR3


@time_fun
def get_P(lead1, lead2, lead3, intervals, r_pos, chars, pr1, pr2, pr3, marr_amp_p1, marr_amp_p2, marr_amp_p3, mean_PR, mean_PR1, mean_PR2, mean_PR3):
    global bl, al, k
    # bl = [0.00475052, 0.01425157, 0.01425157, 0.00475052]
    # al = [1.0, -2.25008508, 1.75640138, -0.46831211]
    # bl = [0.01591456, 0.03182911, 0.01591456]
    ...
    for i in range(4, len(r_pos)):
        ...
        range_interval = intervals[i] * 0.36  # 0.36
        if range_interval < mean_PR1:
            mean_PR1_copy = int(range_interval * 0.6)  # 0.6
        if range_interval < mean_PR2:
            mean_PR2_copy = int(range_interval * 0.6)
        if range_interval < mean_PR3:
            mean_PR3_copy = int(range_interval * 0.6)
        ...
        if not (chars[i] == 'N'):
            ...
            if True:
                fragment_l1_f = filtfilt(bl, al, fragment_l1)[:]  # [5:-3]
                ...
                fragment_l2_f = filtfilt(bl, al, fragment_l2)[:]
                ...
                fragment_l3_f = filtfilt(bl, al, fragment_l3)[:]
...
def main():
    ...
    # std_fintervals = get_win_std(fintervals, 5)
    # p.plot(std_fintervals, pen="w")
    inds_min = get_inds_min_diff(intervals)
    pos_p1, pos_p2, pos_p3, mean_p1, mean_p2, mean_p3, mean_PR, mean_PR1, mean_PR2, mean_PR3 = get_p_pos(lead1, lead2, lead3, intervals, r_pos, chars,
                                                                           inds_min)
    coef_p = get_P(lead1, lead2, lead3, intervals, r_pos, chars, pos_p1, pos_p2, pos_p3, mean_p1, mean_p2, mean_p3,
                   mean_PR, mean_PR1, mean_PR2, mean_PR3)
    # p.plot(coef_p, pen='m')
    ...
    # p.plot(clean_fintervals - coef_fibr, pen='r')
    ...
    e_coef_p = coef_p * coef_fibr / clean_fintervals * 1.0 # 0.3
    ...
    # p_count_coef = round(e_coef_p[e_coef_p > clean_fintervals].size / e_coef_p.size, 2)
    p_count_coef = round(e_coef_p[e_coef_p > 1.0].size / e_coef_p.size, 6)
    ...
    print(f"coef2 = {mean_coef_p2:.2f}, coef = {mean_coef_p:.2f}, coef3 = {mean_coef_p3:.2f}")
    # print(f"coef2 = {coef2:.2f}, coef = {coef:.2f}, coef3 = {coef3:.2f}")
    print(f"sum coef = {mean_coef_p2 + mean_coef_p + mean_coef_p3:.2f}")
    norm_coef = 1.0
    # if coef3 > 1.0:
    #     e_coef_p = truncate_ch(e_coef_p, 0.7)
    ...
    shift = 100
    gain = 100
    p.plot(e_coef_p * gain + shift, pen='r', fillLevel=200.0, brush=(60, 180, 60, 140))
    # p.plot(e_coef_p[:int(len(e_coef_p) * 1.0)] * gain + shift, pen='r', fillLevel=200.0, brush=(60, 180, 60, 100))
    threshold_line = np.ones(len(e_coef_p)) * gain + shift
    p.plot(threshold_line, pen='g')
    # p.plot(mean_line * gain + shift, pen='w')
    # p.plot(mean_line2 * gain + shift, pen='w')
    # p.plot(mean_line3 * gain + shift, pen='w')
